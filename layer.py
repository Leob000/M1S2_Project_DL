...
class Film(nn.Module):
    """creates a FiLM layer

    Args:
        gamma (float): multiplicative coefficient of the FiLM layer
        beta (float): additive coefficient of the FiLM layer 
    """
    def __forward__(self, gamma, beta):
        gammas = gamma.unsqueeze(2).unsqueeze(3).expand_as(x)
        betas = beta.unsqueeze(2).unsqueeze(3).expand_as(x)
